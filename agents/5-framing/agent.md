...
summary_format: "Resumo 5"
---

# Agente 5 — Framing do Problema
...
Entradas: Resumo 4 (evidencias + mapa de dores) + contexto de negocio (metas, restricoes, OKRs).
...
**Leia o slug do frontmatter do resumo compacto do estagio anterior** (`outputs/workflow/4-pesquisa/compact/{slug}--*--pesquisa--resumo.md`) e reutilize. Padrao: `{slug}--{YYYYMMDD}--{agent-name}--{artifact-name}.ext` — ver `docs-workflow/templates/process-artifact-schemas.md`.
...
1. **Problem Statement** (TMPL-006) → salve em `outputs/workflow/5-framing/full/`
2. **Metricas de Sucesso** (TMPL-007) → salve em `outputs/workflow/5-framing/full/`
...
## Ao finalizar
Gere o **Resumo 5** (ver `docs-workflow/templates/process-summary-format.md`) e salve em `outputs/workflow/5-framing/compact/` como `{slug}--{YYYYMMDD}--framing--resumo.md`. Propague o `slug` no frontmatter.
